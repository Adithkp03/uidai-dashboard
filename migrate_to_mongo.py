...
def load_data_from_csv():
    """
    Loads data from the 'data/' directory, same logic as original app.py
    """
...
def clean_state_names(df):
    # 1. Standardize text format
    df['state'] = df['state'].astype(str).str.strip().str.title()
    df['state'] = df['state'].str.replace(r'\s+', ' ', regex=True) # Remove extra spaces
    
    # 2. Fix specific typos/variations map
    typo_map = {
        ...
        # Address city-as-state issues if mapped, otherwise they get dropped by whitelist
        'Raja Annamalai Puram': 'Tamil Nadu', # Map known cities if clear, else drop
        'Madanapalle': 'Andhra Pradesh',
        'Puttanahalli': 'Karnataka',
        'Nagpur': 'Maharashtra',
    }
    df['state'] = df['state'].replace(typo_map)
    
    # 3. Strict Whitelist Filter
    # Filter only valid states. This removes random cities or garbage.
...
def preprocess_data(df_enrolment, df_demographic, df_biometric):
    """
    Aggregates raw data to District-Month level and computes Total metrics.
    """
    print("Preprocessing and Aggregating data...")
    
    # Date Parsing Helper
    def parse_dates(df):
        # Format in CSVs seems to be dd-mm-yyyy based on view_file output
        df['date'] = pd.to_datetime(df['date'], format='%d-%m-%Y', errors='coerce')
        # Normalize to Month Start to handle daily/weekly data differences check (e.g. 02-03 vs 01-03)
        df['date'] = df['date'].apply(lambda x: x.replace(day=1) if pd.notnull(x) else x)
        return df

    # --- Enrolment Processing ---
    df_enrolment = parse_dates(df_enrolment)
    # Summing age groups to get total enrolment
    df_enrolment['Total_Enrolment'] = (
        df_enrolment['age_0_5'] + 
        df_enrolment['age_5_17'] + 
        df_enrolment['age_18_greater']
    )
    # Aggregate by Date, State, District (summing counts)
    enrol_agg = df_enrolment.groupby(['date', 'state', 'district'])[['Total_Enrolment']].sum().reset_index()

    # --- Demographic Processing ---
    df_demographic = parse_dates(df_demographic)
    # Summing age groups
    df_demographic['Total_Demographic'] = (
        df_demographic['demo_age_5_17'] + 
        df_demographic['demo_age_17_']
    )
    demo_agg = df_demographic.groupby(['date', 'state', 'district'])[['Total_Demographic']].sum().reset_index()

    # --- Biometric Processing ---
    df_biometric = parse_dates(df_biometric)
    # Summing age groups
    df_biometric['Total_Biometric'] = (
        df_biometric['bio_age_5_17'] + 
        df_biometric['bio_age_17_']
    )
    bio_agg = df_biometric.groupby(['date', 'state', 'district'])[['Total_Biometric']].sum().reset_index()
    ...
    # Fill NaN with 0 (operational assumption: no data = no activity)
    merged.fillna(0, inplace=True)

    # --- Data Cleaning ---
    merged = clean_state_names(merged)

    # Remove rows where state or district are numeric (likely header/parsing artifacts)
    def is_valid_name(x):
        return isinstance(x, str) and not x.replace('.', '', 1).isdigit()
    
    # Also clean District names roughly
    ...
    # Re-aggregate after cleaning to merge duplicates (e.g. 'west bengal' becoming 'West Bengal')
    # Use sum for numerical columns
